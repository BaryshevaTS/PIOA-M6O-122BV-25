...
def test_read_optional_int_float_error(monkeypatch, tui):
    inputs = iter(["12.5", "100"])
    monkeypatch.setattr("builtins.input", lambda _: next(inputs))
...
def test_add_record_via_tui(monkeypatch, tui):
    inputs = iter(["books", "1984", "Orwell", "1949", "328"])
    monkeypatch.setattr("builtins.input", lambda _: next(inputs))
    tui._add_record()
...
def test_show_all_records(monkeypatch, tui, capsys):
    tui.db.create_record("books", title="Test", author="Author", year=2020, pages=100)
...
def test_print_records_multiple(capsys):
    records = [{"id": 1, "title": "A"}, {"id": 2, "title": "B"}]
    DatabaseTUI._print_records(records)
    captured = capsys.readouterr()
...
def test_update_record_via_tui(monkeypatch, tui, capsys):
    tui.db.create_record("books", title="Old", author="Auth", year=1990, pages=200)
...
def test_delete_record_via_tui(monkeypatch, tui, capsys):
    tui.db.create_record("books", title="Del", author="Auth", year=2000, pages=50)
    inputs = iter(["books", "1"])
    monkeypatch.setattr("builtins.input", lambda _: next(inputs))
    tui._delete_record()
...
def test_sort_records_view(monkeypatch, tui, capsys):
    tui.db.create_record("books", title="B", author="Auth1", year=2000, pages=100)
    tui.db.create_record("books", title="A", author="Auth2", year=1990, pages=200)
...
def test_tui_sort_invalid_field(monkeypatch, tui, capsys):
    tui.db.create_record("books", title="Test", author="Auth", year=2020, pages=100)
...
def test_tui_update_no_fields_selected(monkeypatch, tui, capsys):
    tui.db.create_record("books", title="Old", author="Auth", year=1990, pages=100)
    inputs = iter(["books", "1", "", "", "", ""])
    monkeypatch.setattr("builtins.input", lambda _: next(inputs))
    tui._update_record()
    captured = capsys.readouterr()
...
def test_tui_delete_record_error(monkeypatch, tui, capsys):
    tui.db.create_record("books", title="Test", author="Auth", year=2020, pages=100)
    inputs = iter(["books", "999"])
    monkeypatch.setattr("builtins.input", lambda _: next(inputs))
    tui._delete_record()
    captured = capsys.readouterr()
...
def test_tui_create_index_view_unsupported(monkeypatch, tui, capsys):
    monkeypatch.setattr("builtins.input", lambda _: next(iter(["books", "year"])))
    tui._create_index_view()
    captured = capsys.readouterr()
    assert "не поддерживает индексы" in captured.out
